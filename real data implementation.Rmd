...
```{r}
start_time_smcf <- Sys.time()  # Record start time

library(glmnet)
library(MASS) # For true multivariate normal distributions if needed

# Set seed, choose number of observations and number of simulations
#set.seed(8)
n <- nrow(data_clean)
folds <- 5

# Now vectors to save the results from the folds
...
fold_indices <- sample(rep(1:folds, length.out = n))
    ...
    data_0_test$intendedCS <- 0
        ...
        #predictions_on_test_data_smcf <- predict(GLM1_on_train_data_smcf, newdata = data_test[, -23], type = "response")
        ...
        # Propensity score model on training data and test data
...
```{r}
start_time_real <- Sys.time()  # Record start time
...
end_time_real <- Sys.time()  # Record end time
total_time_real <- end_time_real - start_time_real  # Compute total duration
print(total_time_real)  # Print the total computation time
```
